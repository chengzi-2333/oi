...
std::vector<int> h, v, rec;

signed main() {
    #ifndef ONLINE_JUDGE
    freopen("launch-site.in", "r", stdin);
    #endif // ONLINE_JUDGE
    // std::cin.tie(nullptr)->sync_with_stdio(false);
    IO::mmap_init();
    IO::fast_read_u(n);
    h.resize(n+1), v.resize(n+1), rec.resize(n+1);
    for (int i = 1; i <= n; i++) {
        IO::fast_read_u(h[i], v[i]);
        while (!st.empty() && h[st.top()] < h[i]) {
            rec[i] += v[st.top()];
            st.pop();
        }
        if (!st.empty()) rec[st.top()] += v[i];
        st.push(i);
    }
    IO::fast_write_u(*std::max_element(rec.begin(), rec.end()));
    IO::put('\n');
    IO::flush();
}
