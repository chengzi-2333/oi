...
signed main() {
    ...
    IO::fast_read_u(t, n);
    mat.resize(n);
    for (auto& m: mat) IO::fast_read_u(m.wa);
    for (auto& m: mat) IO::fast_read_u(m.wb);
    for (auto& m: mat) IO::fast_read_u(m.v);
    std::sort(mat.begin(), mat.end());
    for (const auto& [wa, wb, v]: mat) {
        for (int j = t; j >= v; j--) {
            f[j] = std::max(f[j], f[j-v] + wa - j * wb);
            ans = std::max(ans, f[j]);
        }
    }
    IO::fast_write_u(ans);
