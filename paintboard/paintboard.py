    ...
    try:
        ...
        if "data" in result:
            ...
            if data.get("errorType"):
                logger.error(f"获取Token失败: {data['errorType']}")
                return None
            return data.get("token")
        return None
    except Exception as e:
        logger.warning(f"获取Token时出错: {str(e)}")
        return None
...
def read_image_original_size(image_path: str) -> Tuple[Optional[List[List[Dict]]], bool]:
    ...
    try:
        with Image.open(image_path) as img:
            ...
            logger.info(f"图片原始尺寸: {original_width}x{original_height}")
            
            # 校验尺寸是否符合画板最大限制（1000x600）
            if not (0 < original_width <= 1000 and 0 < original_height <= 600):
                logger.warning(f"图片尺寸({original_width}x{original_height})超出画板限制（最大1000x600）")
                return None, False
            ...
            logger.info(f"成功读取图片：尺寸{original_width}x{original_height}，总像素{original_width*original_height}个")
            return pixels, True
...
class PaintboardClient:
        ...
        while self.connected:
            ...
            if queue_size > 0 and self.ws and self.ws.state == State.OPEN:
                try:
                    ...
                    if packet_count >= 1:  # 有包就发送
                        ...
                        async with self.send_lock:
                            if len(self.paint_queue) > 0:  # 再次检查队列
                                ...
                                logger.debug(f"发送粘包数据: {len(self.paint_queue)} 字节，包含 {sent_packets} 个绘画指令")
                    ...
                    logger.warning(f"发送绘画请求失败: {str(e)}")
    ...
    async def _parse_message(self, data: bytes):
        ...
        while offset < total_len:
            ...
            # 处理绘画结果（反馈成功/冷却等状态）
                ...
                logger.info(f"AccessKey: {self.access_key}; 绘画结果 [ID:{paint_id}]: {status_msg}")
                # 如果是冷却状态，增加等待时间
                if status == 0xEE:  # 冷却中
            ...
            # 未知操作码
                ...
                logger.warning(f"收到未知操作码: {op}，跳过")
                break
    ...
    async def paint_pixel(self, x: int, y: int, r: int, g: int, b: int) -> Optional[int]:
        ...
        if not (0 <= x < 1000 and 0 <= y < 600):
            logger.warning(f"坐标({x}, {y})超出画板范围，跳过")
            return None
        
        # 校验颜色值（0-255）
        if not (0 <= r <= 255 and 0 <= g <= 255 and 0 <= b <= 255):
            logger.warning(f"颜色值({r},{g},{b})无效，跳过")
            return None
    ...
    async def worker(self):
        ...
        try:
            ...
            if self.ws and self.ws.state == State.OPEN:
                await self.ws.close()
            logger.info(f"客户端已关闭，总共发送了 {self.packet_counter} 个数据包")
    ...
    async def maintainer(self, pixels_map: Dict[Tuple[int, int], Tuple[int, int, int]], 
        ...
        try:
            ...
            logger.info(f"维护者开始监听区域 ({start_x},{start_y}) 到 ({start_x+width-1},{start_y+height-1})")
                        ...
                        if (start_x <= task.x < start_x + width and 
                            ...
                            if (task.x, task.y) in pixels_map:
                                ...
                                if (expected_r != task.r or 
                                    ...
                                    logger.info(f"检测到像素被修改: ({task.x},{task.y}) "
            ...
            if self.ws and self.ws.state == State.OPEN:
                await self.ws.close()
            logger.info(f"维护客户端已关闭，总共发送了 {self.packet_counter} 个数据包")
...
async def populate_task_queue(pixels: List[List[Dict]], start_x: int, start_y: int, ignore_white: bool = False):
    ...
    if (start_x < 0 or start_y < 0 
        ...
        logger.warning(f"绘制范围超出画板！起始({start_x},{start_y})+尺寸({target_width}x{target_height}) > 1000x600")
        return

    logger.info(f"开始填充任务队列：起始位置({start_x},{start_y})，尺寸{target_width}x{target_height}")
    ...
    logger.info(f"任务队列填充完成，共添加 {task_count} 个任务")
...
async def main():
    ...
    with open("paintboard.yml", "r") as f:
