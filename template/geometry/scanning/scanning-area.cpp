...
inline int pos(int val) {
    return std::lower_bound(disc.begin(), disc.end(), val) - disc.begin();
}

inline void discrete() {
    std::sort(disc.begin() + 1, disc.end());
    disc.erase(std::unique(disc.begin() + 1, disc.end()), disc.end());
}
...
template <typename M>
inline M middle(M l, M r) {
    return l + ((r - l) >> 1);
};

void pushup(int p, int l, int r) {
    if (cnt[p])
        len[p] = disc[r + 1] - disc[l];
    else
        len[p] = (l == r) ? 0 : len[lc(p)] + len[rc(p)];
}
...
signed main() {
    ...
    long long py = std::get<0>(edge[0]), sum = 0;
