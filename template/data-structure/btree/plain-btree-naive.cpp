...
class BTree {
   ...
   public:
    ~BTree() { Node<T>::free_node(root); }

    size_t get_size(Node<T>* node) { return node ? node->t_size : 0; }

    void update_size(Node<T>* node) {
        if (node == nullptr) return;
        node->t_size =
            node->count + get_size(node->left) + get_size(node->right);
    }
    ...
    Node<T>* find_node(T key) { return find_node(key, root); }

    Node<T>* insert(T key) { return insert(key, root); }

    Node<T>* erase(T key) { return erase(key, root); }

    size_t get_size(T key) { return get_size(key, root); }

    size_t rank(T key) { return rank(key, root); }

    T kth(size_t k) { return kth(k, root); }

    T lower(T key) { return lower(key, root); }

    T upper(T key) { return upper(key, root); }
};
